			...
			<div class="row articulo wow fadeInRight" data-wow-duration="1s">
				<div class="col-xs-12 col-md-8 col-md-offset-2 article-container">
					<form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST" name="login" enctype="multipart/form-data">
						<input type="text" name="titulo" placeholder="Titulo del Artículo" class="col-xs-12" required>
						<textarea name="texto" placeholder="Texto del Artículo" class="col-xs-12" required></textarea>
						<input type="file" name="blog_img" class="button col-xs-12 button-article" required>
						<input type="submit" value="Crear Articulo" class="button col-xs-12 col-md-2 button-article">
					</form>
				</div>
			</div>
		</div>
